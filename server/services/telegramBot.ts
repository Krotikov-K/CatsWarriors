...
export class TelegramBotService {
  ...
  private setupCommands() {
    ...
    this.bot.onText(/\/start/, async (msg) => {
      ...
      if (existingUser) {
        await this.bot?.sendMessage(chatId, 
          `🐱 Добро пожаловать обратно в Клыки и Клятвы, ${existingUser.username}!\n\n` +
        ...
        try {
          ...
          await this.bot?.sendMessage(chatId,
            `🐱 Добро пожаловать в Клыки и Клятвы, ${userName}!\n\n` +
    ...
    this.bot.onText(/\/play/, async (msg) => {
      ...
      await this.bot?.sendMessage(chatId,
        "🐱 Добро пожаловать в Клыки и Клятвы!\n\nНажмите кнопку ниже, чтобы начать игру:",
        options
      );
    });
    ...
    this.bot.onText(/\/help/, async (msg) => {
      const chatId = msg.chat.id;
      await this.bot?.sendMessage(chatId,
        "🎮 Команды Клыки и Клятвы:\n\n" +
        "/start - начать игру\n" +
        "/play - открыть веб-интерфейс игры\n" +
        "/status - показать статус персонажа\n" +
        "/help - эта справка\n\n" +
        "Клыки и Клятвы - это ММО РПГ по вселенной 'Коты Воители'. Создайте своего кота-воителя и присоединитесь к одному из четырех племен!"
      );
    });
